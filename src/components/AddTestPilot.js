...
class AddTestPilot extends Component {
  ...
  async handleAdd(e) {
    ...
    //sends new test pilot data to db
  ...
  // toggles checkbox and state value
  handleCheck(e) {
    this.setState({ ...this.state, admin: e.target.value })
  }

  // cancels test pilot submission and returns to test pilot list page
  handleCancel(e) {
    e.preventDefault()
    this.props.history.push('/testpilots')
  }

  // save state as fields are input
  onChange(e, key){
